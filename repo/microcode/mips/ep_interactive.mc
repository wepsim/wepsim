...
begin
{
    ...
    mrti:   # notify: skip1st:true showas:offcanvas glow:btn_run_stop_exebar1 scroll2current:true showdetails:control_memory
            # notify: Before each fetch.
            # notify: After executing any instruction, and before the fetch microcode,
            # notify: the CPU checks if any interruption is enable. 
            # notify: 
            # notify: If so, the microcode gets the indentificator of the interruption 
            # notify: from the device itself, and from this identificator the CPU gets 
            # notify: the associated subrutine index in RT1:
            # notify: <font color=blue>if (INT)</font>
            # notify: <font color=blue>&nbsp;&nbsp;&nbsp;&nbsp;MBR <= DB <= INTV</font>
            # notify: <font color=blue>&nbsp;&nbsp;&nbsp;&nbsp;RT1 <= MBR</font>
            # notify: In this CPU, it gets the interruption vector from the device
            # notify: (a number that identify the interruption)
            # notify: and use it as index in the subrutine vector in RT1 register.
            # notify: It also sends an ACK to the device to stop sending the INT signal.
            # notify: 
            # notify: Note: PC already points to next instructions after the interruption
  ...
  csw_rt1:  # notify: skip1st:true showas:offcanvas glow:btn_run_stop_exebar1 scroll2current:true showdetails:control_memory
            # notify: The event handler microsubrutine.
...
reti {
        ...
        {
            # notify: skip1st:true showas:offcanvas glow:btn_run_stop_exebar1 scroll2current:true showdetails:control_memory
...
div reg1 reg2 reg3 {
       ...
       help='r1 = r2 / r3',
       {
               ...
               # notify: skip1st:true showas:offcanvas glow:btn_run_stop_exebar1 scroll2current:true showdetails:control_memory
               # notify: The FPE-DIV0 exception.
               # notify: The 'div r1 r2 r3' microcode checks if r3 == 0 before divide at the ALU.
               # notify: If so (r3==0) there is an associated event -> Floating Point Exception.
               # notify: 
               # notify: In this case, the microcode sets the associated event identification in RT1:
               # notify: <font color=blue>RT1 = 1</font>
               # notify: Then, microjump to event handler microsubrutine:
               # notify: <font color=blue>go csw_rti</font>
               # notify: 
               # notify: Rembember: PC already points to next instructions after syscall
...
bge reg reg offset {
       ...
       help='if ($r1 >= $r2) pc += 4*offset',
       {
           ...
           (T2, C5),
           (SE=1, OFFSET=0, SIZE=10000, T3, C4),
           (MA=1, MB=10, MC=1, MC=1, SELCOP=1100, T6, C4),
           (MA=1, MB=1, MC=1, SELCOP=1010, T6, C2, A0=1, B=1, C=0),
 bck4ftch: (T5, M7=0, C7),
           (A0=1, B=1, C=0)
       }
