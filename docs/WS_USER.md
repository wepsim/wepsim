...
+ And to 'run' 'microstep by microstep' the 'asm-mips_ep_s1e1.txt' assembly for the 'ep' architecture with the 'mc-ep_mips_base.txt' microcode, and print for each microinstruction the state elementes that modify its value:
...
+ You can check if the state at the end of the execution is the same as the one stored on file 'cl-mips_ep_s1e1.txt'. You can 'run' the 'asm-mips_ep_s1e1.txt' assembly for the 'ep' architecture with the 'mc-ep_mips_base.txt' microcode (**and if it matchs the expected state then the output is going to be**):

```bash
./wepsim.sh -a  check -m ep -f ./examples/microcode/mc-ep_mips_base.txt -s ./examples/assembly/asm-mips_ep_s1e1.txt -ok ./examples/checklist/cl-mips_ep_s1e1.txt
...
```bash
./wepsim.sh -a  check -m ep -f ./examples/microcode/mc-ep_mips_base.txt -s ./examples/assembly/asm-mips_ep_s1e1.txt -ok ./examples/checklist/cl-mips_ep_s1e2.txt
