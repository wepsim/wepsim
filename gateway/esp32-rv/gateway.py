...
def do_flash_request(request):
		try:
				...
				# flashing steps...
				log = open('tmp_output.txt', 'a')
				result = subprocess.Popen(['./flash.sh'], stdout=log, stderr=log, shell=True)
				if result.returncode != 0:
						req_data['error']  = result.returncode
						req_data['status'] = 'Error flashing...\n'
						return jsonify(req_data)
...
def do_get_cancel(request):
		try:
				req_data = request.get_json()
				result = subprocess.run(['pkill', 'python'], capture_output=True)
				if result.returncode != 0:
						req_data['error']  = result.returncode
						req_data['status'] = 'Error cancelling...\n'
						return jsonify(req_data)
