 ...
 */
	 ...
	 */
        ...
        poc_states.IOSR = { name: "IOSR",    visible:false, nbits: "32", value: 0, default_value: 0, draw_data: [] };
        poc_states.IOCR = { name: "IOCR",    visible:false, nbits: "32", value: 0, default_value: 0, draw_data: [] };
        poc_states.IODR = { name: "IODR",    visible:false, nbits: "32", value: 0, default_value: 0, draw_data: [] };
         ...
         */
         ...
         poc_signals.INT        = { name: "INT",    visible: true, type: "L", value: 0, default_value:0, nbits: "1", 
                                   depends_on: ["CLK"],
         ...
         poc_signals.IORDY      = { name: "IORDY",  visible: true, type: "L", value: 0, default_value:0, nbits: "1", 
                                   depends_on: ["CLK"],
         ...
         poc_signals.IO_IOR     = { name: "IO_IOR", visible: true, type: "L", value: 0, default_value:0, nbits: "1", 
         ...
         poc_signals.IO_IOW     = { name: "IO_IOW", visible: true, type: "L", value: 0, default_value:0, nbits: "1", 
         ...
         poc_signals.IO_IE       = { name: "IO_IE", visible: true, type: "L", value: 1, default_value: 1, nbits: "1", 
                                    behavior: ["NOP", "IO_CHK_I CLK INT INTV; FIRE C"],
                                    fire_name: [], 
                                    draw_data: [[], []], 
                                    draw_name: [[], []] };

         poc_signals.INTA        = { name: "INTA",  visible: true, type: "L", value: 1, default_value: 0, nbits: "1", 
                                        ...
                                        operation: function (s_expr) 
                                                   {
                                                      ...
                                                      set_value(poc_internal_states.io_int_factory[iocr_id].period, iodr_id);
                                                      set_value(poc_internal_states.io_int_factory[iocr_id].probability, 1) ;
                                                      if (0 == iodr_id)
                                                          set_value(poc_internal_states.io_int_factory[iocr_id].probability, 0) ;
                                                   }
                                      };
        ...
        poc_behaviors.IO_CHK_I       = { nparameters: 4, 
						      ...
						      for (var i=poc_internal_states.io_int_factory.length-1; i>=0; i--)
                                                      {
                                                           if (get_value(poc_internal_states.io_int_factory[i].period) == 0)
 							       continue;

                                                           if (get_value(poc_internal_states.io_int_factory[i].active) == true)
                                                           {
                                                               set_value(poc_signals[s_expr[2]], 1); // ['INT']=1
                                                               set_value(poc_states[s_expr[3]], i);  // ['INTV']=i
                                                           }

                                                           if ((clk % get_value(poc_internal_states.io_int_factory[i].period)) == 0)
                                                           {
                                                              if (Math.random() > get_value(poc_internal_states.io_int_factory[i].probability))
                                                                  continue ;

                                                              set_value(poc_internal_states.io_int_factory[i].accumulated, get_value(poc_internal_states.io_int_factory[i].accumulated) + 1);
                                                              set_value(poc_internal_states.io_int_factory[i].active, true) ;
        ...
        poc_behaviors.INTA           = { nparameters: 6, 
						      ...
						      for (var i=0; i<poc_internal_states.io_int_factory.length; i++) 
                                                      {
                                                           if (get_value(poc_internal_states.io_int_factory[i].active))
                                                           {
