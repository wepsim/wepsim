 ...
 */
	 ...
	 */
        ...
        sim.poc.internal_states.io_int_factory[0] = { period: 0, probability: 0.5, accumulated: 0, active: false } ;
        sim.poc.internal_states.io_int_factory[1] = { period: 0, probability: 0.5, accumulated: 0, active: false } ;
        sim.poc.internal_states.io_int_factory[2] = { period: 0, probability: 0.5, accumulated: 0, active: false } ;
        sim.poc.internal_states.io_int_factory[3] = { period: 0, probability: 0.5, accumulated: 0, active: false } ;
        sim.poc.internal_states.io_int_factory[4] = { period: 0, probability: 0.5, accumulated: 0, active: false } ;
        sim.poc.internal_states.io_int_factory[5] = { period: 0, probability: 0.5, accumulated: 0, active: false } ;
        sim.poc.internal_states.io_int_factory[6] = { period: 0, probability: 0.5, accumulated: 0, active: false } ;
        sim.poc.internal_states.io_int_factory[7] = { period: 0, probability: 0.5, accumulated: 0, active: false } ;
                                        ...
                                        operation: function (s_expr) 
                                                   {
                                                      ...
                                                      if (0 == iodr_id)
                                                          set_var(sim.poc.internal_states.io_int_factory[iocr_id].probability, 0) ;
                                                   },
                                           verbal: function (s_expr) 
                                                   {
                                                      var verbal = "" ;
        ...
        sim.poc.behaviors.IO_CHK_I  = { nparameters: 4, 
						      ...
						      for (var i=sim.poc.internal_states.io_int_factory.length-1; i>=0; i--)
                                                      {
                                                           if (get_var(sim.poc.internal_states.io_int_factory[i].period) == 0)
 							       continue;

                                                           if (get_var(sim.poc.internal_states.io_int_factory[i].active) == true)
                                                           {
                                                               set_value(sim.poc.signals[s_expr[2]], 1); // ['INT']=1
                                                               set_value( sim.poc.states[s_expr[3]], i); // ['INTV']=i
                                                           }

                                                           if ((clk % get_var(sim.poc.internal_states.io_int_factory[i].period)) == 0)
                                                           {
                                                              if (Math.random() > get_var(sim.poc.internal_states.io_int_factory[i].probability))
                                                                  continue ;
                                        ...
                                        operation: function (s_expr) 
                                                   {
                                                      ...
                                                      if (typeof sim.poc.events.io[clk] != "undefined") 
                                                      {
                                                          set_value(sim.poc.states[s_expr[4]], sim.poc.events.io[clk][0]); // ['BUS_DB'] = i
  							  return ;
                                                      }

						      set_value(sim.poc.signals[s_expr[2]], 0); // ['INT']  = 0
						      set_value(sim.poc.states[s_expr[5]], 0); // ['INTV'] = 0

						      for (var i=0; i<sim.poc.internal_states.io_int_factory.length; i++) 
                                                      {
                                                           if (get_var(sim.poc.internal_states.io_int_factory[i].active))
                                                           {
                                                               set_value(sim.poc.signals[s_expr[2]], 0) ; // ['INT']  = 1
                                                               set_value(sim.poc.states[s_expr[5]], i) ; // ['INTV'] = i
							       set_value(sim.poc.states[s_expr[4]], i) ; // ['BUS_DB'] = i
