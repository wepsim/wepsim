...
+ From the command line it is possible to 'run' the 'asm-mips_ep_s1e1.txt' assembly for the 'ep' architecture with the 'mc-ep_mips_base.txt' microcode, and print the final state:
...
+ And to 'run' 'microstep by microstep' the 'asm-mips_ep_s1e1.txt' assembly for the 'ep' architecture with the 'mc-ep_mips_base.txt' microcode, and print for each microinstruction the state elements that modify its value:
...
+ You can check if the state at the end of the execution is the same as the one stored on file 'cl-mips_ep_s1e1.txt'. You can 'run' the 'asm-mips_ep_s1e1.txt' assembly for the 'ep' architecture with the 'mc-ep_mips_base.txt' microcode (**and if it matches the expected state, then the output is going to be**):
...
+ You can check if the state at the end of the execution is the same as the one stored on file 'cl-mips_ep_s1e1.txt'. You can 'run' the 'asm-mips_ep_s1e1.txt' assembly for the 'ep' architecture with the 'mc-ep_mips_base.txt' microcode (**and if it fails to match the expected state then the output is going to be**):

```bash
./wepsim.sh -a check -m ep -f ./examples/microcode/mc-ep_mips_base.txt -s ./examples/assembly/asm-mips_ep_s1e1.txt -r ./examples/checklist/cl-mips_ep_s1e2.txt
...
```bash
./wepsim.sh -a microstepverbalized -m ep -f ./examples/microcode/mc-ep_mips_base.txt -s ./examples/assembly/asm-mips_ep_s1e1.txt
...
!./wepsim-2.1.8/wepsim.sh -a stepbystep -m ep -f ./wepsim-2.1.8/examples/microcode/mc-ep_mips_base.txt -s ./wepsim-2.1.8/examples/assembly/asm-mips_ep_s1e1.txt > ./result.csv
!rm -fr   wepsim-2.1.8
