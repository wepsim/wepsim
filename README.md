...
+ For example (Thanks to both, Javier Prieto Cepeda and Saúl Alonso Monsalve for several examples):
  ...
  2. Then you can load the example 12 of WepSIM (that comes with the basic MIPS instruction set) by clicking on https://wepsim.github.io/wepsim/ws_dist/wepsim-classic.html?mode=ep&example=12
  3. Finally just substitute the assembly code from the example 12 with the generated by Compiler Explorer, and compile it. The code can execute right from WepSIM.
