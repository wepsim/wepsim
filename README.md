...
+ For example (Thanks to both, Javier Prieto Cepeda and Saúl Alonso Monsalve for several examples):

  1. First you can go to Compiler Explorer Web Page and load directly an example by clicking on https://godbolt.org/z/1EQHur
     ![screen:assembly1](https://raw.githubusercontent.com/acaldero/wepsim/master/images/examples/compiler-explorer-example.png)
  2. Then you can load the example 14 of WepSIM (that comes with the basic MIPS instruction set) by clicking on https://wepsim.github.io/wepsim/ws_dist/wepsim-classic.html?mode=ep&example=14
  3. Finally just substitute the assembly code from the example 14 with the generated by Compiler Explorer, and compile it. The code can execute right from WepSIM.
