...
function assembly_oc_eoc_v1_1 ( machineCode, oc, eoc )
{
...
function assembly_replace_v1_1 ( machineCode, num_bits, startbit, stopbit, bits, free_space )
{
...
function read_data_v1_1 ( context, datosCU, ret )
{
...
function read_text_v1_1 ( context, datosCU, ret )
{
	   //
	   //  .text
	   //
...
// </TOCHECK>
		...
		else
		{
			...
			machineCode = assembly_oc_eoc_v1_1(machineCode,
		...
		for (i=0; i<binaryAux[candidate].length; i++)
		{
			...
			// replace instruction and fields in machine code
                        ...
                        {
			    machineCode = assembly_replace_v1_1(	machineCode,
...
function simlang_compile_v1_1 (text, datosCU)
{
	  ...
	  text_found = false;
          ...
          while (!is_end_of_file(context))
          {
		   ...
		   read_data_v1_1(context, datosCU, ret);
		   data_found = true;
	       }

	       if ("text" == ret.seg[segname].kindof) {
		   read_text_v1_1(context, datosCU, ret);
		   text_found = true;
	       }
