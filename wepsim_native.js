 ...
 */
    ...
    {
        return (get_value(simhw_sim_signals()[elto]) >>> 0) ;
    }

    function wepsim_native_set_signal ( elto, value )
    {
        set_value(simhw_sim_signals()[elto], value) ;
    ...
    function wepsim_native_get_value ( component, elto )
    {
        ...
        if ( ("CPU" == component) || ("BR" == component) )
        {
            ...
            if (isNaN(index))
                return (get_value(simhw_sim_states()[elto]) >>> 0) ;
        ...
        if ("DEVICE" == component)
        {
            ...
            var value = (get_value(simhw_sim_states()[associated_state]) >>> 0) ;

            set_value(simhw_sim_states().BUS_AB, elto) ;
            set_value(simhw_sim_signals().IOR, 1) ;
            compute_behavior("FIRE IOR") ;
            value = get_value(simhw_sim_states().BUS_DB) ;
    ...
    function wepsim_native_set_value ( component, elto, value )
    {
        ...
        if ( ("CPU" == component) || ("BR" == component) )
        {
            ...
            if (isNaN(index)) 
            {
                set_value(simhw_sim_states()[elto], value) ;
                if ("REG_PC" == elto) {
                    show_asmdbg_pc() ;
		}
        ...
        {
            var associated_state = io_hash[elto] ;
            set_value(simhw_sim_states()[associated_state], value) ;

            set_value(simhw_sim_states().BUS_AB, elto) ;
            set_value(simhw_sim_states().BUS_DB, value) ;
            set_value(simhw_sim_signals().IOW, 1) ;
    ...
    function wepsim_native_get_field_from_ir ( fields, index )
    {
        ...
        var value = get_value(simhw_sim_states().REG_IR) ;
    ...
    {
        set_value(simhw_sim_states().MUXA_MICROADDR, maddr) ;
    }

    function wepsim_native_go_opcode ( )
    {
	var maddr = get_value(simhw_sim_states().ROM_MUXA) ;
        set_value(simhw_sim_states().MUXA_MICROADDR, maddr) ;
    }
